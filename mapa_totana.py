...
function vigilarFondo(mapa,capas){
  ...
  capas[0].on('tileerror',function(){
    ...
    cambiado=true;
    capas.forEach(function(c){ if(mapa.hasLayer(c)) mapa.removeLayer(c); });
...
function llInitMapa(){
  ...
  var base=L.tileLayer(ESRI_GRIS,{attribution:'© Esri',maxNativeZoom:16,maxZoom:18}).addTo(llMapa);
  ...
  // Etiquetas en un panel por encima del campo de lluvia: si fueran parte
  // del fondo quedarían tapadas por la mancha de color.
  var etiq=L.tileLayer(ESRI_GRIS_ETIQ,{pane:'lletiq',maxNativeZoom:16,maxZoom:18}).addTo(llMapa);
  vigilarFondo(llMapa,[base,etiq]);
