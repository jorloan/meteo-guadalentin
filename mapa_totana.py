...
L.control.layers(
  ...
  {'Radar lluvia':rLG,'Mapa calor':hLG,'Marcadores':mLG},
...
function render(){
  ...
  leg.upd(p);
  hLG.clearLayers(); mLG.clearLayers(); HL=null;
  ...
  // Heatmap
  if(!isR&&feats.length>2){
    try{
      var c=turf.featureCollection(feats);
      var g=turf.interpolate(c,2.5,{gridType:'square',property:'value',units:'kilometers',weight:p==='temp'?2:4});
      ...
      HL=L.geoJSON(cl,{pane:'hp',style:function(f){
        return{fillColor:col(f.properties.value,p),fillOpacity:window.HO,stroke:false};
      }});
      hLG.addLayer(HL);
